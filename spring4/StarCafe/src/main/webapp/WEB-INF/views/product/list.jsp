...
<body>
	<div class="container">
				...
				<tbody>
					...
					<c:forEach var="item" items="${list}">
						<tr>
							...
							<td>${item.status_}</td>
							<td><a href="update/${item.id}" class="btn btn-sm btn-info">변경</a>
								<a href="delete/${item.id}" class="btn btn-sm btn-secondary">삭제</a></td>
						</tr>
					</c:forEach>
				</tbody>
							...
							<ol class="pagination pagination-sm justify-content-center">
								...
								<c:forEach var="page" items="${pager.list}">
									<li class="page-item ${page eq pager.page ? 'active' : ''}"><a
										href="?page=${page}${pager.query}" class="page-link">${page}</a></li>
								</c:forEach>
		...
		<div class="mt-2">
			<a href="add" class="btn btn-sm btn-success">등록</a> <a href="list"
				class="btn btn-sm btn-danger">이전</a>
		</div>

		<div class="mt-2">
			<a href="dummy" class="btn btn-sm btn-primary">대량등록</a> <a
				href="init" class="btn btn-sm btn-warning">초기화</a>
		</div>

		<div class="mt-2">
			<a href="/" class="btn btn-sm btn-dark">처음으로</a>
		</div>
	</div>
</body>
</html>